...
int main(int argc, char** argv)
{
  ...
  for (const auto& file : directory_iterator(config.input_dir)) {
    ...
    if (!core::utils::get_pair_paths(file.path().string(), eo_path, ir_path)) {
      continue;
    }
    ...
    save_path += core::utils::extract_file_name(eo_path);

    cout << "\n========================================" << endl;
    cout << "Processing: " << core::utils::extract_file_name(eo_path) << endl;
    cout << "========================================" << endl;

    // 根據檔案類型處理
    if (core::utils::is_video(eo_path)) {
      cout << "[MODE] Video Processing" << endl;
      processor.process_video(eo_path, ir_path, save_path);
    } else {
      cout << "[MODE] Image Processing" << endl;
      processor.process_image(eo_path, ir_path, save_path);
    }

    // 顯示計時結果
    processor.show_timer_results();
  }
