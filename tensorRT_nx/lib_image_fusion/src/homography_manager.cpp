...
HomographyManager::HomographyManager(double max_trans_diff, double max_rot_diff, double alpha)
...
std::pair<double, double> HomographyManager::calculateDifference(const cv::Mat& homo1,
...
bool HomographyManager::shouldUpdate(const cv::Mat& new_homo) const {
    if (previous_homo_.empty()) {
        return true;
    }
    
    auto [trans_diff, rot_diff] = calculateDifference(previous_homo_, new_homo);
...
cv::Mat HomographyManager::update(const cv::Mat& new_homo) {
    ...
    // 判斷是否應該更新
    if (shouldUpdate(new_homo)) {
        // 平滑混合
        cv::Mat smoothed = smooth_alpha_ * new_homo + (1 - smooth_alpha_) * previous_homo_;
...
cv::Mat HomographyManager::getCurrent() const {
    return previous_homo_;
}

void HomographyManager::setParameters(double max_trans_diff, double max_rot_diff, double alpha) {
    max_translation_diff_ = max_trans_diff;
    max_rotation_diff_ = max_rot_diff;
    smooth_alpha_ = alpha;
}
