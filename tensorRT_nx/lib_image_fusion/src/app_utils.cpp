...
double calcFeaturePointMSE(const cv::Mat& homo_pred, const cv::Mat& homo_gt,
    ...
    for (const auto& pt : eo_pts) {
        eo_pts_f.push_back(cv::Point2f(static_cast<float>(pt.x), static_cast<float>(pt.y)));
    }
...
cv::Mat refineHomographyWithRANSAC(std::vector<cv::Point2i>& eo_pts,
    ...
    for (const auto& pt : ir_pts) {
        ir_pts_f.push_back(cv::Point2f(static_cast<float>(pt.x), static_cast<float>(pt.y)));
    }
    
    cv::Mat mask;
    cv::Mat H = cv::findHomography(eo_pts_f, ir_pts_f, cv::RANSAC, ransac_threshold, mask, 3000, 0.99);
