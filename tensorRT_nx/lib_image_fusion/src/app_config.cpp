...
void AppConfig::init_defaults(nlohmann::json& config)
{
  ...
  // Pipeline 控制參數 - 新增
  config.emplace("align_start_frame", 15);
  config.emplace("align_stop_frame", -1);
  config.emplace("align_on_first_frame", true);
}

void AppConfig::parse_from_json(const nlohmann::json& config)
{
  // 輸入輸出
...
bool AppConfig::load(const std::string& config_path)
{
  ...
  if (!std::filesystem::is_regular_file(config_path)) {
    std::cout << "\033[1;31m[ ERROR ]\033[0m Config file not found: "
  ...
  try {
    ...
    // 解析配置
    parse_from_json(config);

    return true;
  }
  catch (const std::exception& e) {
    std::cout << "\033[1;31m[ ERROR ]\033[0m Failed to parse config: "
              << e.what() << std::endl;
    return false;
  }
}
...
bool AppConfig::validate() const
{
  ...
  if (!std::filesystem::is_directory(input_dir)) {
    std::cout << "\033[1;31m[ ERROR ]\033[0m Input directory not found: "
              << input_dir << std::endl;
    return false;
  }
  ...
  if (output_enabled && !std::filesystem::is_directory(output_dir)) {
    std::cout << "\033[1;31m[ ERROR ]\033[0m Output directory not found: "
              << output_dir << std::endl;
    return false;
  }
