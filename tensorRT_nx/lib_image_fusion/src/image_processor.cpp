...
bool ImageProcessor::initialize()
{
  ...
  if (!fusion_trt_->is_initialized()) {
    std::cerr << "\033[1;31m[ ERROR ]\033[0m TRT Fusion initialization failed!"
              << std::endl;
...
cv::Mat ImageProcessor::perform_fusion(const cv::Mat& eo_gray,
                                       ...
                                       const cv::Mat& M)
{
  ...
  // Warp EO gray
  cv::Mat eo_warped = utils::warp_with_homography(eo_gray, M, cv::Size(out_w, out_h));
...
cv::Mat ImageProcessor::compute_homography(const cv::Mat& eo,
                                           const cv::Mat& ir,
                                           ...
                                           int frame_cnt)
{
  ...
  if (eo_pts.size() >= 4 && ir_pts.size() >= 4) {
    cv::Mat H = utils::refine_homography_with_ransac(eo_pts, ir_pts, M, 8.0);

    if (!H.empty() && cv::determinant(H) > 1e-6 && cv::determinant(H) < 1e6) {
      // 判斷是否接受新的 homography
      if (homo_manager_.get_current().empty()) {
        M = homo_manager_.update(H);
        homo_manager_.reset_fallback();
        std::cout << "  - Frame " << frame_cnt << ": First homography computed"
                  << std::endl;
      }
      else {
        auto [trans_diff, rot_diff] = homo_manager_.calculate_difference(
            homo_manager_.get_current(), H);
        ...
        if (trans_diff > config_.smooth_max_translation_diff ||
          ...
          homo_manager_.increment_fallback();
          std::cout << "    -> Difference too large, fallback="
                    << homo_manager_.get_fallback_count() << std::endl;

          if (homo_manager_.get_fallback_count() >= 3) {
            std::cout << "    -> Force accept and reset!" << std::endl;
            homo_manager_.reset();
            homo_manager_.set_parameters(config_.smooth_max_translation_diff,
                                         config_.smooth_max_rotation_diff,
                                         config_.smooth_alpha);
            M = homo_manager_.update(H);
          }
          else {
            M = homo_manager_.get_current();
          }
        }
        else {
      ...
      std::cout << "  - Frame " << frame_cnt << ": Poor quality, using previous"
                << std::endl;
    }
  }
  else {
    ...
    std::cout << "  - Frame " << frame_cnt
              << ": Insufficient points, using previous" << std::endl;
  }
  timer_homo_.stop();
...
cv::Mat ImageProcessor::compose_output(const cv::Mat& ir_original,
...
bool ImageProcessor::process_image(const std::string& eo_path,
                                   ...
                                   const std::string& save_path)
{
  ...
  if (config_.picture_cut_enabled) {
    eo = utils::crop_image(eo, config_.pcut_x, config_.pcut_y,
  ...
  // 設定圖片名稱
  std::string img_name = utils::extract_base_name(eo_path);
  ...
  if (config_.use_model_prediction) {
    ...
    // RANSAC 優化
    M = utils::refine_homography_with_ransac(eo_pts, ir_pts, M, 6.0);
    if (M.empty()) {
      M = cv::Mat::eye(3, 3, CV_64F);
    }

    // 儲存 homography 到快取 (覆蓋模式，單一檔案)
    utils::save_homography_to_cache(config_.homo_cache_file, M);
  }
  else {
    // 模式二：從快取載入 homography
    std::cout << "  [MODE] Loading homography from cache" << std::endl;

    M = utils::load_homography_from_cache(config_.homo_cache_file);
    if (M.empty()) {
      std::cerr << "  [ERROR] Failed to load homography from cache, using identity matrix"
                << std::endl;
      M = cv::Mat::eye(3, 3, CV_64F);
    }
  }
  ...
  // Warp EO
  cv::Mat eo_warped = utils::warp_with_homography(eo_resized, M, cv::Size(out_w, out_h));

  // 融合
  cv::Mat gray_eo_warped;
  cv::cvtColor(eo_warped, gray_eo_warped, cv::COLOR_BGR2GRAY);
  cv::Mat fused = perform_fusion(gray_eo_warped, ir_resized, cv::Mat::eye(3, 3, CV_64F));

  // 組合輸出
  cv::Mat output = compose_output(ir_resized, eo_resized, ir_resized, eo_warped, fused);
  ...
  if (config_.use_model_prediction) {
    cv::Mat gt_homo = utils::read_gt_homography(config_.gt_homo_base_path, img_name);
    if (!gt_homo.empty() && !eo_pts.empty()) {
      double mse = utils::calc_feature_point_mse(M, gt_homo, eo_pts);
      std::cout << "    MSE Error: " << mse << " px^2" << std::endl;

      utils::write_error_to_csv("image_homo_errors.csv", img_name, mse);
    }
  }
...
bool ImageProcessor::process_video(const std::string& eo_path,
                                   const std::string& ir_path,
                                   const std::string& save_path)
{
  int out_w = config_.output_width;
  ...
  // 跳過幀數
  utils::skip_frames(eo_path, eo_cap, config_.skip_frames_config);
  utils::skip_frames(ir_path, ir_cap, config_.skip_frames_config);
  ...
  std::cout << "  - Use Model Prediction: "
            << (config_.use_model_prediction ? "true" : "false") << std::endl;
  std::cout << "  - Align Start Frame: " << config_.align_start_frame << std::endl;
  std::cout << "  - Align Stop Frame: " << config_.align_stop_frame
            << " (-1 = never stop)" << std::endl;
  std::cout << "  - Align on First Frame: "
            << (config_.align_on_first_frame ? "true" : "false") << std::endl;
  ...
  if (config_.output_enabled) {
    std::string mode_suffix =
        config_.use_model_prediction ? "_align" : "_fusion_only";
  ...
  cv::Mat cached_homo = utils::load_homography_from_cache(config_.homo_cache_file);
  ...
  std::string video_name = utils::extract_file_name(eo_path);
  ...
  while (true) {
    ...
    if (config_.video_cut_enabled) {
      eo = utils::crop_image(eo, config_.vcut_x, config_.vcut_y,
    ...
    // ===== Pipeline 控制邏輯 =====
    // 判斷當前幀是否需要執行 align
    bool execute_align = should_execute_align(cnt, is_first_frame);
    ...
    if (execute_align) {
      ...
      // 執行 align (model 推論)
      M = compute_homography(img_eo, img_ir, eo_pts, ir_pts, cnt);
      ...
      if (M.empty()) {
        ...
        cv::Mat cached_H = utils::load_homography_from_cache(config_.homo_cache_file);
        if (!cached_H.empty()) {
          ...
          M = cv::Mat::eye(3, 3, CV_64F);
          std::cout << "    -> Using identity matrix (no valid homography)"
                    << std::endl;
        }
      }
    }
    ...
    // ===== 共同處理：Warp + Fusion =====
    // Warp EO
    cv::Mat eo_warped = utils::warp_with_homography(img_eo, M, cv::Size(out_w, out_h));

    // 融合
    cv::Mat fused = perform_fusion(gray_eo, img_ir, M);

    // 組合輸出
    cv::Mat output = compose_output(img_ir, img_eo, img_ir, eo_warped, fused);
    ...
    if (execute_align && !eo_pts.empty()) {
      cv::Mat gt_homo = utils::read_gt_homography_for_frame(
          video_name, cnt, config_.gt_video_base_path);
      if (!gt_homo.empty()) {
        double mse = utils::calc_feature_point_mse(M, gt_homo, eo_pts);
        std::cout << "    [Frame " << cnt << "] MSE: " << mse << " px^2"
                  << std::endl;
        ...
        utils::write_error_to_csv("video_homo_errors.csv", video_name, mse,
                                  extra_cols);
      }
    }
...
void ImageProcessor::show_timer_results()
{
  timer_resize_.show();
  timer_gray_.show();
  timer_align_.show();
...
bool ImageProcessor::should_execute_align(int frame_cnt, bool is_first_frame)
{
